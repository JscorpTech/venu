                ...
                <div class="d-flex flex-wrap gap-3 justify-content-between">
                    ...
                    @if ($seller['status'] == 'pending')
                        <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                            <form class="d-inline-block" action="{{ route('admin.vendors.updateStatus') }}"
                                id="reject-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{ $seller['id'] }}">
                                <input type="hidden" name="status" value="rejected">
                                <button type="button" class="btn btn-danger form-alert"
                            ...
                            <form class="d-inline-block" action="{{ route('admin.vendors.updateStatus') }}"
                                ...
                                <button type="button" class="btn btn-success form-alert"
                                    data-message="{{ translate('want_to_approve_this_vendor') . '?' }}"
                                    data-id="approve-form">{{ translate('approve') }}</button>
                            </form>
                        </div>
                    @endif
                    ...
                    @if ($seller['status'] == 'suspended' || $seller['status'] == 'rejected')
                        <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                            <form class="d-inline-block" action="{{ route('admin.vendors.updateStatus') }}"
                                id="active-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{ $seller['id'] }}">
                                <input type="hidden" name="status" value="approved">
                                <button type="button" class="btn btn-success px-5 form-alert"
                                    data-message="{{ translate('want_to_active_this_vendor') . '?' }}"
                                    data-id="active-form">{{ translate('active') }}</button>
                            </form>
                        </div>
                    @endif
                </div>
                ...
                <div class="d-flex gap-3 flex-wrap flex-lg-nowrap">
                    <div class="border p-3 w-170">
                        <div class="d-flex flex-column mb-1">
                            <h5 class="font-weight-normal">{{ translate('total_products') }} :</h5>
                            <h3 class="text-primary fs-18">{{ $seller->product_count }}</h3>
                        </div>

                        <div class="d-flex flex-column">
                            <h5 class="font-weight-normal">{{ translate('total_orders') }} :</h5>
                    ...
                    <div class="row gy-3 flex-grow-1 w-100">
                        <div class="col-sm-6 col-xxl-3">
                            ...
                            <div class="pair-list">
                                ...
                                <div>
                                    <span class="key">{{ translate('long') }}</span>
                                    <span>:</span>
                                    <span class="value">{{ $seller?->shop->long }}</span>
                                </div>
                                <div>
                                    <span class="key">{{ translate('lat') }}</span>
                                    <span>:</span>
                                    <span class="value">{{ $seller?->shop->lat }}</span>
                                </div>

                                <!-- region and district -->
                                @if ($seller->shop->region && $seller->shop->district)
                                    <div>
                                        <span class="key">{{ translate('region') }}</span>
                                        <span>:</span>
                                        <span class="value">{{ $seller?->shop->region->name }}</span>
                                    </div>
                                    <div>
                                        <span class="key">{{ translate('district') }}</span>
                                        <span>:</span>
                                        <span class="value">{{ $seller?->shop->district->name }}</span>
                                    </div>
                                @endif
